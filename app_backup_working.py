...
class StrategyExtraction:
    ...
    stop_loss: StrategyRule = field(default_factory=lambda: StrategyRule("stop_loss", "Missing", "missing"))
    take_profit: StrategyRule = field(default_factory=lambda: StrategyRule("take_profit", "Missing", "missing"))
    risk_management: StrategyRule = field(default_factory=lambda: StrategyRule("risk_management", "Missing", "missing"))
    position_sizing: StrategyRule = field(default_factory=lambda: StrategyRule("position_sizing", "Missing", "missing"))
    repainting_risk: str = "Unknown"
    missing_information: List[str] = field(default_factory=list)
    subjective_terms: List[str] = field(default_factory=list)
    coding_readiness_score: int = 0
    scam_or_cherry_pick_warning: str = "Not assessed"
    summary: str = ""
...
def get_transcript(video_id: str, languages: Optional[List[str]] = None) -> str:
    ...
    try:
        ...
        try:
            ...
            raise TranscriptUnavailableError("Transcripts are disabled for this video") from exc
...
def find_indicators(text: str) -> List[str]:
    ...
    lowered = text.lower()
    return [name for name, pattern in indicator_patterns.items() if re.search(pattern, lowered)]
...
def detect_market(text: str) -> Optional[str]:
    ...
    if any(word in lowered for word in ["bitcoin", "btc", "ethereum", "eth", "crypto", "usdt"]):
        return "crypto"

    if any(word in lowered for word in ["forex", "eurusd", "gbpusd", "xauusd", "nas100", "us30"]):
        return "forex/cfd"

    if any(word in lowered for word in ["stock", "stocks", "spy", "qqq", "nasdaq", "s&p"]):
...
def extract_rules_with_keywords(text: str) -> StrategyExtraction:
    ...
    for sentence in sentence_split:
        ...
        clarity = "subjective" if any(term in sl for term in subjective_terms) else "vague"

        if any(word in sl for word in ["buy", "long", "enter long"]):
            if any(word in sl for word in ["cross", "above", "below", "greater than", "less than", "%"]):
                clarity = "exact"
        ...
        if any(word in sl for word in ["exit", "close", "sell when", "take profit", "stop loss"]):
            if any(word in sl for word in ["cross", "above", "below", "%", "atr", "risk reward", "r:r"]):
                clarity = "exact"
            exit_rules.append(StrategyRule("exit", s, clarity))

    stop_loss = StrategyRule("stop_loss", "Missing", "missing")
    stop_match = re.search(r"(stop loss|stop-loss|sl)[^.!?]{0,80}", text, flags=re.IGNORECASE)
    if stop_match:
    ...
    tp_match = re.search(r"(take profit|take-profit|tp|target)[^.!?]{0,80}", text, flags=re.IGNORECASE)
    if tp_match:
    ...
    risk_match = re.search(r"(risk|risk management)[^.!?]{0,100}", text, flags=re.IGNORECASE)
    if risk_match:
    ...
    size_match = re.search(r"(position size|position sizing|lot size|size)[^.!?]{0,100}", text, flags=re.IGNORECASE)
    if size_match:
    ...
    elif any(word in lowered for word in ["pivot", "zigzag", "fractal", "future candle"]):
...
"""
    ...
    def rule_from_any(value: Any, fallback_category: str) -> StrategyRule:
        ...
        if isinstance(value, list):
            if not value:
                return StrategyRule(fallback_category, "Missing", "missing")
            normalized_items = [rule_from_any(item, fallback_category) for item in value]
            combined_rule = " | ".join(item.rule for item in normalized_items if item.rule and item.rule != "Missing")
            if not combined_rule:
                combined_rule = "Missing"
            clarity_order = {"exact": 3, "vague": 2, "subjective": 1, "missing": 0}
            best_clarity = max(normalized_items, key=lambda item: clarity_order.get(item.clarity, 0)).clarity
        ...
        if isinstance(value, str):
            return StrategyRule(fallback_category, value, "vague" if value.strip() else "missing")
    ...
    return StrategyExtraction(
        ...
        entry_rules_long=rules_list_from_any(data.get("entry_rules_long", []), "long_entry"),
        entry_rules_short=rules_list_from_any(data.get("entry_rules_short", []), "short_entry"),
        ...
        scam_or_cherry_pick_warning=data.get("scam_or_cherry_pick_warning", "Not assessed"),
        summary=data.get("summary", ""),
        failure_reasons=data.get("failure_reasons", []),
        strategy_type=data.get("strategy_type"),
        pine_script_ready=bool(data.get("pine_script_ready", False)),
        confidence_score=int(data.get("confidence_score", data.get("coding_readiness_score", 0))),
    )
...
def validate_strategy_core(youtube_url: str, transcript_text: Optional[str] = None, save: bool = False) -> StrategyExtraction:
...
if FastAPI is not None and BaseModel is not None:
    ...
    def validate_strategy_api(request: ValidateRequest):
        try:
            result = validate_strategy_core(request.youtube_url, request.transcript_text)
...
class TestYouTubeStrategyValidator(unittest.TestCase):
    ...
    def test_fallback_analysis_penalizes_vague_strategy(self):
        transcript = "Wait for confirmation and enter with momentum. Exit when it feels weak."
        result = extract_rules_with_keywords(transcript)
    ...
    for filename in os.listdir(reports_dir):
        ...
        try:
            ...
            leaderboard.append({
                ...
                "warning": data.get("scam_or_cherry_pick_warning", "Unknown")[:160],
            })
        except Exception:
            continue
...
def main() -> int:
    ...
    parser.add_argument("--batch", help="Text file with YouTube URLs")
